  ...
  <div class="todo-list">
    <div class="content">
      <div class="todoListDiv">
        ...
        <div class="listDiv">
          <ul>
            <li class="task-list-item" v-for="item in todoStore.todoList" :key="item.id">
              <label class="task-list-item-label">
                <el-checkbox v-model="item.complete" />
                <span>{{ item.content }}</span>
              </label>
              <span class="delete-btn" @click="deleteItem(item)"></span>
            </li>
          </ul>
        </div>
      </div>
...
import type { ListItem } from '../store/todo'
...
.is-checked+span {
  color: #5470c6;
  text-decoration: line-through rgba(233, 89, 89, 0.8);
}
